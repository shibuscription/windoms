...
変更点サマリ（最大5行）
- 活動記録一覧の表示形式（`開始時刻～` + 種別本文）と時刻列の揃え方を追記
- 活動記録モーダルの `種別` 必須バリデーションと、エラー表示位置（ラベル行右側）を追記
- 活動記録の曲追加UIを「候補タップ/Enter追加・キャンセルのみ」に更新
- 曲追加の重複防止・未解決時フィードバック（ローカルトースト）を明記
- `songIds` の上限（最大10件）を明記
