...
- 会計担当者が購入した場合: 購入依頼を `BOUGHT` にする際に会計へ支出1件追加可能
- 立替の場合: `DONE` になったタイミングで会計へ支出1件追加
...
- 支払いは `paymentSplits`（支払い内訳）で管理する。
  - `QUO`: `{ type:"quo", cardId, amount }`
  - `立替`: `{ type:"reimbursement", amount }`
  - QUOは複数カード利用を許容し、立替との併用も可とする。
  - 内訳合計は `amount` と一致しなければ確定できない。
  - QUO残高不足は該当行エラーとして扱い、確定不可とする。
...
- `paymentSplits` に `立替` が含まれる場合のみ、立替金額分を Reimbursements へ未精算レコードとして片方向追加する。
  - `source` は `lunch` を設定し、由来を判別できるようにする。
  - 逆同期は行わない。
- `paymentSplits` の QUO行は指定 `cardId` ごとに残高を減算する（active前提にしない）。

### 26.4 QUOカード残高（目安管理）

- QUOカードは配列で保持し、新旧2枚以上の同時存在を許容する。
- active カードを支払い元とする（MVPでは1枚active運用を基本）。
- `admin` のみ Lunch 画面内で QUOカード管理UIを表示する。
  - カード追加（`name`, 初期`balance`）
  - active切替
  - `balance` 手動修正
- 残高は目安管理とし、手動補正を許容する。
- activeカード残高が閾値（例: 2000）未満のときは `admin` のみに注意表示する。
