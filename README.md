...
変更点サマリ（最大5行）
- 第19章前提に `songMaster/{songId}`（`name` 必須）を追記
- 第5.6節に「4桁時刻でも改行しない」要件を追記し、時刻列仕様を整合
- 第20章「現在の実装状況（DEMO段階）」を新設
- 実装済み項目（曲UI・トースト・種別必須・活動記録一覧整形）を明文化
- 未実装 / 今後の大項目（省力化参照・シフト草案・会計・権限制御）を追記
