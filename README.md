...
- イベント詳細から「このイベントのTODO一覧」をモーダル表示する（DEMOでは表示のみで保存しない）。
- イベント詳細で `完了にする / 進行中に戻す` を切り替えできる（DEMOはローカル状態更新）。
- イベント詳細から「紐づきセッション一覧」をモーダル表示し、既存セッション表示UIと統一感を持たせる。
...
- イベント詳細の紐づきセッション一覧から「イベントセッション選択」モーダルを開ける（DEMOは表示のみ、保存なし）。
- 紐づきセッション一覧モーダルには、管理ユーザー向けに「＋（紐づけ）」と「解除」導線を表示する（DEMOは未実装トースト）。
