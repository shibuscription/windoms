...
- 例: お弁当アイコンは当日のコンテキストを引き継ぎ、`/lunch?date=YYYY-MM-DD` へ遷移する。
- DEMO段階では導線確認を優先し、お弁当アイコンは常時表示としてよい（将来は条件表示）。
...
- 行にはモジュール、列には `利用者区分` / `管理権限` / `担当業務` の各項目を並べ、交点のチェックボックスで表示可否を設定する。
- モジュール名にはアイコンを表示する。
