...
- 締めた期は取引の追加・編集ができない。
- 締め処理時に新しい期を作成する。
- 各口座の期末残高を、新期の期首残高へ自動設定する。
- 期首残高は手修正可能（通常は変更しない）。

##### 締めを解除する

- 締め解除すると、その期は再編集可能になる。
- 締め解除は権限者（会長・会計）のみ実行可能。
- ただし、次の期に取引が存在する場合は締め解除できない（整合性維持のため）。
- 締め解除できるのは、次の期が未使用（取引なし）の場合に限る。
...
- 上段：大きな操作ボタン
  ...
  - －支出
  - ⇄振替
- 中段：口座残高カード一覧（タップで通帳画面）
- 下段：今期収支サマリ（収入合計／支出合計／差額）
- 軽い分析表示（棒グラフ想定、円グラフ必須ではない）
