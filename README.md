...
- `共有TODO` は従来のTODOであり、担当を持てる。セッションまたはイベントに紐付けできる。
- `個人TODO` は作成者本人にしか見えない。担当は持たず、セッション / イベント紐付けも持たない。
- 上部TODOアイコン（受信箱）には「自分担当の未完了共有TODO」と「自分の未完了個人TODO」の両方を表示する。

### 24.2 TODOデータモデル（DEMO最小）

- `kind: 'shared' | 'private'`
  - 未設定の既存データは `shared` として扱う
- `title: string`
- `completed: boolean`
...
- `共有TODO` タブでは、既存の一覧・フィルタ・未アサイン / アサイン済み表示を維持する。
