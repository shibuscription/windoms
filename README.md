...
- 管理者/会計担当（`admin`）は「会計に支出を記録する」チェック（デフォルトON）
- 追加後の追跡・逆同期は行わない
- 管理者/会計担当が購入完了時に会計連携導線を使う場合は、少なくとも `出金元口座 / 科目 / 摘要` を保持できるようにする
...
- 操作ボタンと遷移
  - 未精算:
    ...
    - `支払済にする` は専用モーダルで確認する
      ...
      - チェックON時のみ、会計へ支出記録の追加処理を実行する（片方向・逆同期なし）
      - チェックOFF時は支払済更新のみ実行する
      - 会計連携導線では、少なくとも `出金元口座 / 科目 / 摘要` を指定できるようにする
...
- 購入依頼で直接会計へ記録する場合は、購入完了時に会計へ支出 1 件を追加する
- 立替で会計へ記録する場合は、`支払済` にする時点で会計へ支出 1 件を追加する
- お弁当で直接会計へ記録する場合は、登録時に会計へ支出 1 件を追加する
...
- 会計連携の将来実装に備えて、購入依頼・立替・お弁当の各レコードは少なくとも以下の情報を保持できる構造にする。
  - `accountingRequested` または同等の、会計起票の希望・準備状態
  - `accountingLinked`
  - `accountingEntryId`
